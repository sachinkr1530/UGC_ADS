...
import { a, button, div, img, p } from "framer-motion/client";
import { Ellipsis, ImageIcon, Loader2Icon, PlaySquareIcon, Share2Icon, Trash2Icon } from "lucide-react";
import { GhostButton, PrimaryButton } from "./Buttons";

const ProjectCard = ({ gen, setGenerations, forCommunity = false }: {
    gen: Project,
    setGenerations: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?:
    boolean
}) => {
                ...
                <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' :
                    ...
                    {gen.generatedImage && (
                        <img src={gen.generatedImage} alt={gen.productName} className=
                            {`absolute insert-0 w-full h-full object-cover transition 
                    ...
                    {gen.generatedVideo && (
                        <video src={gen.generatedVideo} muted loop playsInline
                            className="absolute insert-0 w-full h-full object-cover transition 
                    duration-500"
                    ...
                    {(!gen?.generatedImage && !gen?.generatedVideo) && (
                        <div className="absolute insert-0 w-full h-full flex flex-col 
                items-center justify-center bg-black/20  ">
                  ...
                  group-hover:opacity-100 transition flex items-center gap-2 ">
                            <div className="absolute top-3 right-3">
                                <Ellipsis className="ml-auto bg-black/10 rounded-full p-1 size-7" />
                            </div>
                        ...
                        rounded-lg shadow-md mt-2 py-1 z-10`}>
                              ...
                              hover:bg-blak/10 cursor-pointer">
                              ...
                              hover:bg-blak/10 cursor-pointer">
                                    ...
                                    <button onClick={() => handleDelete(gen.id)} className="w-full flex gap-2 items-center
                               px-4 py-2 hover:bg-red-950/10 text-red-400
                                cursor-pointer">
                ...
                <div className="p-4">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            {gen.updatedAt && (
                                <p className="text-xs text-gray-500 mt-1">Updated: {new Date(gen.createdAt).toLocaleDateString()} </p>
